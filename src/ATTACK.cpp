...
////退弹标志位
const float PID_P[5] = {0.3,0,0,3000,2600};
const float PID_S[5] = {4.5,0,0,3000,1816};
...
int YUN_F_JAM( )
{
    ...
    if ((ATTACK[0].DATA.SPEED_NOW )< 200 &&(single_lock == 1))
    {
        jam_time ++ ;
    }
    if ((ATTACK[0].DATA.SPEED_NOW )< 200 &&(dbus_data.REMOTE.s1_u8 == 2))
    {
        jam_time ++ ;
    }
...
//        if ((ATTACK[0].DATA.SPEED_NOW < 200) && (dbus_data.REMOTE.s1_u8 == 1) )
//        {
//            jam_time ++ ;
////            jam_angle = ATTACK[0].DATA.ANGLE_INFINITE;
//        }
//        (dbus_data.REMOTE.s1_u8 ==2)
...
//
        if (YUN_F_JAM())//没堵弹
        {
            ...
            {
                single_lock = 0;
                ATTACK[0].DATA.AIM = (float )ATTACK[0].DATA.ANGLE_INFINITE;//存疑，我觉得目标值下降是因为这句话，电机还没转到目标值程序就运行到这，然后把目标值更新为中途的角度
...
//        if (dbus_data.REMOTE.s1_u8 == )
            ...
            if (dbus_data.REMOTE.s1_u8 == 1)//单发不上锁
            {

                if (single_lock == 0)
                {
                    ATTACK[0].DATA.AIM = (float) ATTACK[0].DATA.ANGLE_INFINITE + 1800050.0f;
                    single_lock = 1;
                }
...
//            }
//            && (jam_time < 500)
            else if (dbus_data.REMOTE.s1_u8 == 2)//连发
            {
...
//            }
            ...
            YUN_F_MOTOR_PID_GIMBAL(&ATTACK[0], (float)ATTACK[0].DATA.ANGLE_INFINITE, ATTACK[0].DATA.SPEED_NOW);
